    ...
    with sess.as_default():
        ...
        for epoch in six.moves.xrange(args.nb_epochs):
            ...
            for batch in range(nb_batches):
                ...
                feed_dict = {x: X_train[start:end], y: Y_train[start:end], K.learning_phase(): 1}
                if feed is not None:
...
def model_train_1(sess, x, y, model, X_train, Y_train, generator, save=False,
                x_advs=None, init_all=True, evaluate=None,
                verbose=True, feed=None, args=None):
    """
    Train a TF graph
    :param sess: TF session to use when training the graph
    ...
    if x_advs is not None:
        ...
        predictions_adv = model(tf.stack(x_advs)[idx])
        loss = (loss + model_loss(y, predictions_adv)) / 2
    ...
    with sess.as_default():
        ...
        for epoch in six.moves.xrange(args.nb_epochs):
            ...
            for (batch_data, batch_labels) \
                    in generator.flow(X_train, Y_train, batch_size=args.batch_size):

                if len(batch_data) < args.batch_size:
                    k = args.batch_size - len(batch_data)
                ...
                if step % 100 == 0:
                    print('Minibatch loss: ', curr_loss)
                    evaluate()
                    sys.stdout.flush()
                step += 1
                if step == nb_batches:
...
def model_eval(sess, x, y, predictions=None, X_test=None, Y_test=None,
    ...
    with sess.as_default():
        ...
        for batch in range(nb_batches):
            ...
            feed_dict = {x: X_test[start:end], y: Y_test[start:end], K.learning_phase(): 1}
            if feed is not None:
                feed_dict.update(feed)
...
def tf_model_load2(sess):
    ...
    with sess.as_default():
        saver = tf.train.Saver()
        saver.restore(sess, os.path.join(FLAGS.train_dir, FLAGS.filename1))

    return True


def tf_load_model(sess, train_dir, filename):
    """
    ...
    with sess.as_default():
        saver = tf.train.Saver()
        saver.restore(sess, os.path.join(train_dir, filename))
